...
saveButton.addEventListener('click', () => {
    ...
    if (title && body) { // Проверяем, что поля заголовка и текста не пустые.
        ...
        // В этом случае, мы можем сохранить body как есть, если оно уже включает переводы строк
        // const formattedBody = body.split('\n').map(line => line.trim()).join('\n'); // Убираем лишние пробелы и оставляем переносы строк
        ...
        if (editingNote) { // Если редактируем существующую заметку
            ...
            if (index !== -1) {
                ...
                const updatedNote = { title, body: formattedBody, color }; // Используем formattedBody
            ...
            notes.push({ title, body: formattedBody, color }); // Используем formattedBody
